...
// ===== SITE CONFIG =====
const WHATSAPP_NUMBER = '919072301300';
const SHOW_SIZE_GUIDE = false;
var SITE_URL = 'https://vimayasarees.com';
var INSTAGRAM_HANDLE = '@vimaya_by_remya';
var DELIVERY_ITEMS = [];
...
fetch('data/products.json')
  ...
  if (item.sizes) {
    ...
    sizeArr.forEach(function(s) {
      ...
      btn.type = 'button';
      btn.textContent = sz;
      btn.style.cssText = 'padding:6px 14px;border:1.5px solid #1A1A1A;border-radius:20px;background:transparent;color:#1A1A1A;font-size:0.8rem;cursor:pointer;transition:all 0.2s;font-weight:500;';
      btn.addEventListener('click', function() {
        selectedSize = sz;
  ...
  waShareBtn.addEventListener('click', function(e) {
    e.stopPropagation();
    var msg =
      '\uD83C\uDF38 Hey! Check out this beautiful saree from Vimaya!\n\n' +
      '*' + item.name + '*\n' +
  ...
  igShareBtn.addEventListener('click', function(e) {
    ...
    var caption =
      ...
      '\uD83D\uDCB0 Price: ' + item.price + '\n\n' +
      '\uD83D\uDED2 Shop: ' + SITE_URL + '\n\n' +
      '#VimayaSarees #SareeLove #TraditionalSaree #SilkSaree #IndianFashion #WhatsAppShopping';
    openIgModal(caption);
  });
    ...
    if (site.catalogue_enabled && catalogueSection) {
      ...
      if (site.catalogue_mode === 'whatsapp_business') {
        ...
        var catMsg = site.catalogue_message ||
          'Hi Vimaya! I\'d like to receive your full saree catalogue. Please send it to me. Thank you!';
